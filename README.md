...
æ†‡※‽⁁⁂⁄⁊⁑⁒⁜₠₡₢₣₤₥₦₧₨₩₪₫₭₮₯₰₱₲₳₴₵₶₷₹₺₻₼₽₾℀℁ℂ℄℅℆ℇ℈ℊℋℌℍℎℏℐℑℒℓ℔ℕ№℗℘ℙℚℛℜℝ℞℟℣ℤ℥Ω℧ℨ℩KÅℬℭ℮ℯℰℱℲℳℴℵℶℷℸ℺℻ℼℽℾℿ⅀⅁⅂⅃⅄ⅅⅆⅇⅈⅉ⅊⅋⅌⅍ⅎ⅏αβγδεζηθικλμνξοπρςστυφχψωↀↁↂↃↄ⦀∀∁∂∃∄∅∆∇∈∉∏∐∑∓∔√∛∜∝∞∥∦∧∨∩∪∫∮∯∱∲∳≨≩≪≫≭≮≯≰≱≲≳≴≵≶≷≸≹≺≻≼≽≾≿⊀⊁⊂⊃⊄⊅⊆⊇⊈⊉⊊⊋⊌⊍⊎⊏⊐⊑⊒⊓⊔⊕⊖⊗⊘⊙⊚⊛⊜⊝⊞⊟⊠⊡⊢⊣⊤⊥⊦⊧⊨⊩⊪⊫⊬⊭⊮⊯⊰⊱⊴⊵⊺⊻⊼⊽⋂⋃⋄⋇⋈⋉⋊⋋⋌⋍⋎⋏⋐⋑⋒⋓⋔⋕⋖⋗⋚⋛⋜⋝⋞⋟⋠⋡⋢⋣⋤⋥⋦⋧⋨⋩⋪⋫⋬⋭⋲⋳⋴⋵⋶⋷⋸⋹⋺⋻⋼⋽⋾⋿⟁⟂⟃⟄⟇⟐⟑⟒⟓⟔⟕⟖⟗⟘⟙⟚⟛⟠⟡⟢⟣⟤⟥⟦⟧⟬⟭⦁⦂⦃⦄⦅⦆⦇⦈⦉⦊⦑⦒⦓⦔⦕⦖⦰⦱⦲⦳⦴⦵⦶⦷⦸⦹⦺⦻⦼⦽⦾⦿⧀⧁⧂⧃⧄⧅⧆⧇⧈⧉⧊⧋⧌⧍⧎⧏⧐⧖⧘⧙⧚⧛⧜⧝⧞⧡⧣⧤⧥⧧⧬⧮⧰⧲⧴⧺⧻⨁⨂⨄⨅⨆⨇⨈⨉⨊⨋⨌⨍⨎⨏⨐⨑⨒⨓⨔⨕⨖⨗⨘⨙⨚⨛⨜⨝⨞⨠⨢⨣⨤⨥⨦⨧⨨⨭⨮⨯⨰⨱⨲⨳⨴⨵⨶⨷⨸⨹⨺⨻⨿⩀⩁⩂⩃⩄⩅⩆⩇⩈⩉⩌⩍⩎⩏⩐⩑⩒⩓⩔⩕⩖⩙⩚⩛⩜⩝⩞⩟⩠⩡⩢⩣⩤⩥⩦⩧⩨⩩⩫⩬⩭⩮⩯⩰⩱⩲⩳⩷⩸⩹⩺⪅⪆⪇⪈⪉⪊⪋⪌⪍⪎⪏⪐⪑⪒⪓⪔⪕⪖⪗⪘⪙⪚⪛⪜⪝⪞⪟⪠⪡⪢⪣⪤⪥⪦⪧⪨⪩⪪⪫⪬⪭⪮⪯⪰⪱⪲⪵⪶⪷⪸⪹⪺⪻⪼⪽⪾⪿⫀⫁⫂⫃⫄⫅⫆⫇⫈⫉⫊⫋⫌⫏⫐⫑⫒⫓⫔⫕⫖⫗⫘⫙⫚⫛⫝⫝⫞⫟⫠⫡⫢⫣⫤⫥⫦⫧⫨⫩⫪⫫⫬⫭⫲⫳⫴⫵⫷⫸⫹⫺⫻⫼⫽⫾⫿⌀⌁⌂⌅⌆⌇⌈⌉⌊⌋⌕⌖⌗⌘⌤⌥⌦⌧⌫⌬⌭⌮⌯⌰⌱⌲⌶⌷⌸⌹⌺⌻⌼⌽⌾⌿⍀⍁⍂⍃⍄⍅⍆⍇⍈⍉⍊⍋⍌⍍⍎⍏⍐⍑⍒⍓⍔⍕⍖⍗⍘⍙⍚⍛⍜⍝⍞⍟⍠⍡⍢⍣⍤⍥⍦⍧⍨⍫⍬⍭⍮⍯⍱⍲⍸⍻⍼⍽⍾⍿⎀⎁⎂⎃⎄⎅⎆⎇⎈⎉⎊⎋⎌⎍⎎⎏⎐⎑⎒⎓⎔⎕⎗⎘⎙⎚⎶⏀⏁⏂⏃⏄⏅⏆⏇⏈⏉⏊⏋⏌⏍⏎⏚⏛⏣⏧①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳ⒶⒷⒸⒹⒺⒻⒼⒽⒾⒿⓀⓁⓃⓄⓅⓆⓇⓈⓉⓊⓋⓌⓍⓎⓏⓐⓑⓒⓓⓔⓕⓖⓗⓘⓙⓚⓛⓜⓝⓞⓟⓠⓡⓢⓣⓤⓥⓦⓧⓨⓩ⓪□▢▤▥▧▨▯△▷▻▽◁◅◇◈◊◍◎◫◬◯◰◱◲◳◴◵◶◷☆☇☈☉☊☋☌☍☏☐☒☓☖☜☞☟☡☤☥☧☨☩☫☬☭☼☽☾☿♁♃♄♅♆♔♕♖♗♘♙♡♢♤♧♩♪♫♬♭♮♯♰♱♲♺⚀⚁⚂⚃⚄⚅⚆⚇⚎⚏⚐⚘⚚⚝⚢⚣⚤⚥⚦⚨⚩⚬⚭⚮⚯⚲⚳⚴⚵⚶⚷⚸⚺⚻⚼⚿⛉⛋⛍⛐⛒⛕⛗⛙⛜⛝⛞⛠⛡⛢⛣⛤⛫⛭⛮⛯⛶⛻⛼⛿✁✃✄✆✇✑✓✗✙✛✜✞✟✢✣✤✥✧✩✫✬✭✮✯✰✲✵✺✻✽✾❀❁❂❃❅❆❈❉❊❋❍❏❐❑❒⬠⬡⬦⬨⬭⬯⬰⬱⬲⭀⭁⭂⭃⭄⭅⭆⭔⭘ᚌᚍᚎᚏⰀⰁⰂⰃⰄⰅⰆⰇⰈⰉⰊⰋⰌⰍⰎⰏⰐⰒⰓⰔⰕⰖⰗⰘⰙⰚⰛⰜⰝⰞⰟⰠⰡⰢⰣⰤⰥⰦⰧⰨⰩⰪⰫⰬⰭⰮⰰⰱⰲⰳⰴⰵⰶⰷⰸⰹⰺⰻⰼⰽⰾⰿⱀⱁⱂⱃⱄⱅⱆⱇⱈⱉⱊⱋⱌⱍⱎⱏⱐⱑⱒⱓⱔⱕⱖⱗⱘⱙⱚⱛⱜⱝⱞᚠᚡᚢᚣᚤᚥᚦᚧᚨᚩᚪᚫᚬᚭᚮᚯᚱᚳᚴᚵᚶᚷᚸᚹᚺᚻᚼᚾᛄᛈᛉᛊᛋᛏᛒᛓᛔᛕᛖᛗᛘᛝᛞᛟᛠᛡᛢᛣᛤᛥᛦᛨᛩᛪ᛭ᛮᛯᛰ
